<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<div class="modal modal-blur fade" id="polluserChooseModal" tabindex="-1" data-keyboard="false" data-backdrop="static"
...
var originCol;
$('.pollUserChoice').on('mouseenter', function( event ) {
	originCol = $(this).css("background")
    $(this).css("background","lightblue");
}).on('mouseleave', function( event ) {
	$(this).css("background",originCol);
})
$(".pollUserChoice").on("click", function () {
	$("#rrsearchResult").empty()
	$("#rrAddList").empty()
	$("#polluserChooseModal").modal("show")
	$("#receiverRefModalTitle").text("설문조사 대상설정")
	var zTree2;
var demoIframe2;
...
var jsonArray2
$.ajax({
	url : "<%=request.getContextPath()%>/edoc/getdeptList",
	type : "post",
	async:false,
...
$("#polluserChooseModal").on("change", "#allCheck3", function() {
...
$("#rrListAddBtn").on("click", function() {
	...
	$.each(addList, function(i, ob) {
		...
		console.log(id)
		console.log(name)
		code += '<span class="badge badge-dark mx-1" role="'+id+'">'+name+'&nbsp;<i class="fas fa-user-times" onclick="deleteUser(this)"></i></span>'
	})
	$("#pollUserInput").empty()
	$("#pollUserInput").append(code)
	
})
function deleteUser(user) {
	$(user).closest("span").remove();
	
	var cnt = $("#pollUserInput").find("span")
	if(cnt.length == 0){
		$("#pollUserInput").append("대상자를 선택해주세요..")
	}
}
</script>
