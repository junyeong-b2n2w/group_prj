			...
			onPageClick : function (res) {	// 페이지네이션 후 받을데이터 처리
				if(res != null && res.length != 0){
					...
					$.each(res, function(i, obj) {
						...
						code += "<td><a href='javascript:OpenWindow(\"detail.do?edno="+obj.ed_no+"\",\"전자문서"+obj.ed_no+"\",700, 850)' style='color: deepskyblue;text-decoration: revert;'>"+obj.ed_aprvtitle+"</a></td>"
						...
						}else if(obj.ed_aprvstt == "DED02"){
							code += '<td>임시저장</td><td><i class="fas fa-pause-circle h4 text-muted mr-1 ml-3"></i></td>'
						...
						}else if(obj.ed_aprvstt == "DED06"){
							code += '<td>회수</td><td><i class="fas fa-minus-circle h4 text-warning mr-1 ml-3"></i></td>'
						}
						code += "</tr>"
					})
				}else{
					code = "<tr><td colspan='6' class='text-center'><h4>조회 결과가 없습니다.</h4></td></tr>"
				}
...
// 			radio : {col : "ntc_writerid", name : "inmap"}, // 라디오 (col:컬럼명, name:인풋태그네임)
		...
		test.setPagenation("aprvListPaging","savePageList", 1) // (페이지네이션 어펜드할곳, 컨트롤러URL, 1(필수
})
$(document).ready(function() {
	var mainCheck = "${mainCheck}"
	if(mainCheck == "main"){
		$("#sttSelect").val("DED05").prop("selected", true)
	}
