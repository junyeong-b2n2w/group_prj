...
// 			radio : {col : "ntc_writerid", name : "inmap"}, // 라디오 (col:컬럼명, name:인풋태그네임)
		...
		test.setPagenation("aprvListPaging","edocPageList", 1) // (페이지네이션 어펜드할곳, 컨트롤러URL, 1(필수
})
$(document).ready(function() {
	var mainCheck = "${mainCheck}"
	if(mainCheck == "main1"){
		$("#sttSelect").val("DED01").prop("selected", true)
	}else if(mainCheck == "main2"){
		$("#sttSelect").val("DED03").prop("selected", true)
	}else if(mainCheck == "main3"){
		$("#sttSelect").val("DED04").prop("selected", true)
	}
