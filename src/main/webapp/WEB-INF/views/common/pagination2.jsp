	...
	function Pagination(setting) {
		...
		function setpage(ajaxUrl, pageAreaId, pageMaker) {
			if(pageMaker && pageMaker.length != 0){
				...
				if(!pageMaker[0].isPrev){
					code +="disabled"
				}
				code += "' value='"+(pageMaker[0].startPage-1)+"'><a class='page-link' href='#'\r\n" 
				code += "tabindex='-1' aria-disabled='true'> <svg\r\n"
				code += "xmlns='http://www.w3.org/2000/svg' class='icon' width='24'\r\n" 
				code += "height='24' viewBox='0 0 24 24' stroke-width='2'\r\n" 
				code += "stroke='currentColor' fill='none' stroke-linecap='round'\r\n"  
				code += "stroke-linejoin='round'>\r\n" 
				code += "<path stroke='none' d='M0 0h24v24H0z' fill='none'></path>\r\n" 
				code += "<polyline points='15 6 9 12 15 18'></polyline></svg> prev\r\n" 
				code += "</a></li>\r\n" 
				for(i=pageMaker[0].startPage; i<=pageMaker[0].endPage; i++){
					code += "<li class='page-item "
					if(pageMaker[0].page == i){
						code += "active"
					}
					code += "' value='"+i+"'><a class='page-link'>"+i+"</a></li>\r\n" 
				}
				code += "<li class='page-item "
				if(!pageMaker[0].isNext){
					code +="disabled"
				}
				code += "' value = '"+(pageMaker[0].endPage+1)+"'><a class='page-link' href='#'> next <svg\r\n" 
				code += "xmlns='http://www.w3.org/2000/svg' class='icon' width='24'\r\n" 
				code += "height='24' viewBox='0 0 24 24' stroke-width='2'\r\n" 
				code += "stroke='currentColor' fill='none' stroke-linecap='round'\r\n" 
				code += "stroke-linejoin='round'>\r\n" 
				code += "<path stroke='none' d='M0 0h24v24H0z' fill='none'></path>\r\n" 
				code += "<polyline points='9 6 15 12 9 18'></polyline></svg>\r\n" 
				code += "</a></li>\r\n" 
				code += "</ul>\r\n" 
				code += "</div>"
